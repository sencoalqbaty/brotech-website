...
const tawkChatBox = document.getElementById('chat-box');
const tawkInput = document.getElementById('chat-input');
const tawkSendBtn = document.getElementById('chat-send-btn');

const tawkApiKey = '94bfeec9ce7c9d246153ae7c036dc5fc4ad04c';
const propertyId = '68a795d1c874f8192637c011';

// جلب الرسائل
async function fetchTawkMessages() {
    if (!tawkChatBox) return;
    try {
        const res = await fetch(`https://api.tawk.to/v1/inboxes/${propertyId}/messages`, {
            headers: {
        ...
        tawkChatBox.innerHTML = '';

        messages.forEach(msg => {
            const div = document.createElement('div');
            div.classList.add('message', msg.from === 'agent' ? 'admin' : 'client');

            const avatar = document.createElement('img');
            avatar.classList.add('chat-avatar');
            avatar.src = msg.from === 'agent' ? './images/avatar.png' : './images/avatar-client.png';
            avatar.alt = msg.from === 'agent' ? 'Admin' : 'Client';
            ...
            div.appendChild(p);
            tawkChatBox.appendChild(div);
        });

        tawkChatBox.scrollTop = tawkChatBox.scrollHeight;
    } catch (err) {
        console.error('خطأ في جلب رسائل Tawk.to:', err);
    }
}

// إرسال رسالة
async function sendTawkMessage() {
    const message = tawkInput.value.trim();
    if (!message) return;

    try {
        await fetch(`https://api.tawk.to/v1/inboxes/${propertyId}/messages`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${tawkApiKey}`,
                'Content-Type': 'application/json'
        ...
        console.error('خطأ في إرسال رسالة Tawk.to:', err);
    }
}

// أحداث الإرسال
if (tawkSendBtn && tawkInput) {
    tawkSendBtn.addEventListener('click', sendTawkMessage);
    tawkInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') sendTawkMessage();
    });
}

// تحديث الرسائل كل 5 ثواني
setInterval(fetchTawkMessages, 5000);
fetchTawkMessages(); // التحميل الأولي
